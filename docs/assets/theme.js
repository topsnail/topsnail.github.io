document.addEventListener('DOMContentLoaded', function() {    
    ...
    const bgUrlTrimmed = GLOBAL_CONFIG.bgUrl && String(GLOBAL_CONFIG.bgUrl).trim();

    const preloadBackgroundImage = () => {
        if (!bgUrlTrimmed) return;
        const bgImg = new Image();
        bgImg.src = bgUrlTrimmed;
    ...
    const htmlBgCss = bgUrlTrimmed
        ? `background: url(${JSON.stringify(bgUrlTrimmed)}) no-repeat center center fixed !important; background-size: cover !important;`
        : `background: #e9ebe5 !important;`;
    ...
    let finalCss = `
        ...
        @media (max-width: 767px) {
            ...
            .Label { margin-right: 2px !important; }
            .markdown-body {
                font-size: 14px !important;
            }
            .markdown-body img {
